...
public class Part1Application  implements ApplicationRunner {
	...
	public void run(ApplicationArguments args) throws Exception {
		...
		s1.setDatetime(Timestamp.valueOf("2023-01-15 10:00:00"));  
		s1.setDuration(2);
		s1.setTopic("Web Services Fundamentals");
		s1 = sessionRepo.save(s1);

		Module m1 = new Module("CS101", "Intro to Software Systems", 1, false);
		m1.getSessions().add(s1);
		m1 = moduleRepo.save(m1);
        
		Session s2 = new Session();
		s2.setDatetime(Timestamp.valueOf("2023-01-18 13:00:00"));  
		s2.setDuration(2);
		s2.setTopic("Testing Principles");
		s2 = sessionRepo.save(s2);

		Module m2 = new Module("CS201", "Advanced Programming", 2, true);
		m2.getSessions().add(s2);
		m2 = moduleRepo.save(m2);

		Convenor c1 = new Convenor("Dr. Smith", Position.PROFESSOR);
		c1.getModules().add(m1);
		c1 = convenorRepo.save(c1);

		Convenor c2 = new Convenor("Dr. Johnson", Position.LECTURER);
